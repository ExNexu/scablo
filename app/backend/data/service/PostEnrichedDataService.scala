...
trait PostEnrichedDataService extends PostChangeListener {
  ...
  def search(term: String): Future[List[PostEnriched]] = {
    val lowercaseTerm = term.toLowerCase
    // TODO: Can be made more efficient with one par collection and or filters
    val titleAndTagsList = future {
      (enrichedPosts.filter(_.title.toLowerCase.contains(lowercaseTerm)) :::
        enrichedPosts.filter(_.tags.exists(tag => tag.toLowerCase.contains(lowercaseTerm)))).distinct
    }
    val fullTextList = future {
      enrichedPosts.par.filter(_.text.toLowerCase.contains(lowercaseTerm)).toList
    }
    titleAndTagsList flatMap {
      titleAndTagsList =>
        fullTextList map {
          fullTextList => (titleAndTagsList ::: fullTextList).distinct
        }
    }
  }
